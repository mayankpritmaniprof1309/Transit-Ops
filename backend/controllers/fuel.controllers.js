...
export const getFuelLogs = async (req, res, next) => {
  try {
    ...
    res.status(200).json({ success: true, message: 'Fuel logs retrieved successfully', data: fuelLogs });
  } catch (error) {
    next(error);
  }
};
...
export const getFuelLogById = async (req, res, next) => {
  try {
    ...
    res.status(200).json({ success: true, message: 'Fuel log retrieved successfully', data: fuelLog });
  } catch (error) {
    next(error);
  }
};
